...
class ComponentExec:
    ...
    def configQLearning(self):
        ...
        # qLearning 0
        self.qLearning0 = QLearning.QLearning()
        # Acoes
        self.qLearning0.configActions(self.conjDM.list) #qLearning.printActions()
        # Estados
        i=0
        listStates = []
        listStates.append(i)
        ...
        #Configuracoes
        self.qLearning0.configStates(listStates)
        self.qLearning0.configExploration(self.explorationInd)
        self.qLearning0.configReward(self.posReward, self.negReward)
        self.qLearning0.configLearningRate(self.learningRate)
        self.qLearning0.configDiscountFactor(self.discountFactor)
        self.qLearning0.startQTable()
        #qLearning.printQTable()

        # qLearning 1
        self.qLearning1 = QLearning.QLearning()
        self.qLearning1.configActions(self.conjDM.list) #qLearning.printActions()
        self.qLearning1.configStates(listStates)
        self.qLearning1.configExploration(self.explorationInd)
        self.qLearning1.configReward(self.posReward, self.negReward)
        self.qLearning1.configLearningRate(self.learningRate)
        self.qLearning1.configDiscountFactor(self.discountFactor)
        self.qLearning1.startQTable()


        # qLearning 2
        self.qLearning2 = QLearning.QLearning()
        self.qLearning2.configActions(self.conjDM.list) #qLearning.printActions()
        self.qLearning2.configStates(listStates)
        self.qLearning2.configExploration(self.explorationInd)
        self.qLearning2.configReward(self.posReward, self.negReward)
        self.qLearning2.configLearningRate(self.learningRate)
        self.qLearning2.configDiscountFactor(self.discountFactor)
        self.qLearning2.startQTable()

        # qLearning 3
        self.qLearning3 = QLearning.QLearning()
        self.qLearning3.configActions(self.conjDM.list) #qLearning.printActions()
        self.qLearning3.configStates(listStates)
        self.qLearning3.configExploration(self.explorationInd)
        self.qLearning3.configReward(self.posReward, self.negReward)
        self.qLearning3.configLearningRate(self.learningRate)
        self.qLearning3.configDiscountFactor(self.discountFactor)
        self.qLearning3.startQTable()
    ...
    def uploadStudents(self):
        ...
        for x in vetorEstudantesInput:
            ...
            self.listStudents[i].tpStudent = x[5]
            i = i + 1
            #print(x)
            #print( type(x) )
            #print( x[3] )
        ...
        if (self.filaEstudantesAleatoria):
            ...
            while (haEstudantesParaExec): #episodios
                ...
                if (not self.listStudents[nEstudanteSort].finish):
                    ...
                    #
                    if (self.listStudents[nEstudanteSort].tpStudent == 0):
                        idMD = self.qLearning0.recommend(self.listStudents[nEstudanteSort].score, self.listStudents[nEstudanteSort].DMsFinished )
                    if (self.listStudents[nEstudanteSort].tpStudent == 1):
                        idMD = self.qLearning1.recommend(self.listStudents[nEstudanteSort].score, self.listStudents[nEstudanteSort].DMsFinished )
                    if (self.listStudents[nEstudanteSort].tpStudent == 2):
                        idMD = self.qLearning2.recommend(self.listStudents[nEstudanteSort].score, self.listStudents[nEstudanteSort].DMsFinished )
                    if (self.listStudents[nEstudanteSort].tpStudent == 3):
                        idMD = self.qLearning3.recommend(self.listStudents[nEstudanteSort].score, self.listStudents[nEstudanteSort].DMsFinished )
                    ...
                    #
                    if (self.listStudents[nEstudanteSort].tpStudent == 0):
                        self.qLearning0.updateQTable(iStateOld, iStateNew, idMD, trueReward, self.listStudents[nEstudanteSort].DMsFinished)
                    if (self.listStudents[nEstudanteSort].tpStudent == 1):
                        self.qLearning1.updateQTable(iStateOld, iStateNew, idMD, trueReward, self.listStudents[nEstudanteSort].DMsFinished)
                    if (self.listStudents[nEstudanteSort].tpStudent == 2):
                        self.qLearning2.updateQTable(iStateOld, iStateNew, idMD, trueReward, self.listStudents[nEstudanteSort].DMsFinished)
                    if (self.listStudents[nEstudanteSort].tpStudent == 3):
                        self.qLearning3.updateQTable(iStateOld, iStateNew, idMD, trueReward, self.listStudents[nEstudanteSort].DMsFinished)
